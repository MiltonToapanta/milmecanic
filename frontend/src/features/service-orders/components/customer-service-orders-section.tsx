...
import { useServiceOrdersByCustomer } from '../hooks/use-service-orders';
...
export function CustomerServiceOrdersSection({ customerId }: { customerId: string }) {
  const hasPermission = useAuthStore((state) => state.hasPermission);
  const query = useServiceOrdersByCustomer(customerId, { page: 1, limit: 10 });

  return (
    <section className="space-y-4 rounded-md border border-border bg-card p-5">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h2 className="text-base font-semibold">Órdenes de servicio</h2>
          <p className="mt-1 text-sm text-muted-foreground">Trabajos registrados para este cliente.</p>
        </div>
        {hasPermission('service-orders.create') ? (
          <Button>
            <Link to={`/service-orders/new?customerId=${customerId}`} className="flex items-center gap-2">
      ...
      {query.isError ? <ErrorState message="No se pudieron cargar las órdenes del cliente" /> : null}
      {!query.isLoading && !query.isError && (query.data?.items.length ?? 0) === 0 ? <EmptyState title="Este cliente aún no tiene órdenes de servicio" /> : null}
